...
def render_question_preparation_run(subject):
    ...
    run_action = st.radio(
        ...
        horizontal=True,
        key=f"question_prep_action_{subject}",
        help=(
            "Create if missing leaves valid files unchanged. Regenerate for all overwrites all nine "
            "prepared files in every selected topic."
        ),
    )
    if st.button(
