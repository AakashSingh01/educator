...
from question_bank_config import DEFAULT_PREPARATION_WORKERS, MAX_PREPARATION_WORKERS
...
def render_question_preparation_run(subject):
    """Choose a notes subtree and build its reusable question and theory files."""
    st.subheader("Question Preparation Run")
    st.caption(
        "Creates nine reusable JSON files beside every notes.txt: Easy, Medium, and Hard "
        "subjective questions, objective questions, and theory cards."
    )
    scope_key = f"question_prep_scope_{subject}"
    st.session_state.setdefault(scope_key, "")
    scope_mode = st.radio(
        "Preparation scope",
        options=["Whole subject", "Choose a topic or subtopic"],
        horizontal=True,
        key=f"question_prep_mode_{subject}",
        help="A selected topic includes that topic and every descendant subtopic with notes.",
    )
    if scope_mode == "Whole subject":
        ...
        if controls[1].button(
            "Go up one level", key=f"question_prep_up_{subject}", disabled=not selected_scope
        ):
            st.session_state[scope_key] = (
                selected_scope.rsplit("/", 1)[0] if "/" in selected_scope else ""
            )
            st.rerun()
        ...
        if direct_topics:
            topic_filter = st.text_input(
                "Filter direct subtopics",
                placeholder="Type to narrow the list",
                key=f"question_prep_filter_{subject}_{selected_scope}",
            ).casefold()
    ...
    regenerate = st.checkbox(
        "Regenerate existing prepared items",
        key=f"question_prep_regenerate_{subject}",
        help="By default, unchanged valid files are reused. Changed notes are always prepared again.",
    )
    parallel = st.checkbox(
        "Prepare independent topics in parallel",
        value=True,
        key=f"question_prep_parallel_{subject}",
        help="Each topic is generated by a separate process when the configured AI client supports it.",
    )
    worker_options = list(range(1, MAX_PREPARATION_WORKERS + 1))
    default_workers = min(DEFAULT_PREPARATION_WORKERS, MAX_PREPARATION_WORKERS)
    selected_workers = st.selectbox(
        "Parallel workers",
        options=worker_options,
        index=worker_options.index(default_workers),
        disabled=not parallel,
        key=f"question_prep_workers_{subject}",
    )
    if st.button(
        "Start Question Preparation Run",
        type="primary",
        ...
        try:
            with st.spinner("Preparing reusable questions and theory cards. This can take a little while..."):
                summary = backend.prepare_question_banks(
                    subject,
                    selected_scope,
                    max_workers=selected_workers if parallel else 1,
                    overwrite=regenerate,
                )
            st.success(
                f"Question preparation finished: {summary['generated']} topic(s) generated, "
                f"{summary['cached']} unchanged topic(s) reused."
            )
            ...
            if summary["skipped"]:
                st.warning(
                    "Skipped: " + "; ".join(
                        f"{entry.get('topic') or subject} ({entry.get('message')})"
                        for entry in summary["skipped"]
                    )
                )
            if summary["failed"]:
                st.error(
                    "Some topics could not be prepared: " + "; ".join(
                        f"{entry.get('topic') or subject} ({entry.get('message')})"
...
def render_notes_preparation():
    ...
    with st.expander("Prepare reusable questions and theory", expanded=False):
        render_question_preparation_run(subject)

    navigation = st.radio(
        "Notes workflow",
        options=["Continue in-depth", "Choose a topic to edit"],
        horizontal=True,
    ...
    if st.button("Confirm subtopics and continue in-depth", key=f"continue_{safe_topic_key}"):
            ...
            try:
                ...
                st.session_state.last_submit = {
                    ...
                    "info": "\n\n".join(filter(None, (
                        ...
                        f"Why the other options do not fit: {step.get('reason', '')}"
                        if step.get("reason") else "",
                    ))),
                    "submitted": True,
                }
